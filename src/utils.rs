...
pub fn hmh_sketch(file: String, kmer_length: usize, output_name: String) -> Result<(), Box<dyn Error>> {
    let sketch_file = File::open(&file)?;
    ...
    // serialize sketches
    let file_name = format!("{}_sketches.bin",  output_name);
    let mut sketch_writer = BufWriter::new(File::create(file_name)?);

    // compress sketches
    let mut encoder = Encoder::new(sketch_writer, 3).expect("failed to create compression");
    for name in &names {
...
pub fn ull_sketch(precision: u32, sketch_file_name: String, kmer_length: usize, output_name: String) -> Result<(), Box<dyn Error>> {
    ...
    let ull_vec: Vec<UltraLogLog> = files.par_iter().map(|file| {
        let mut ull = Mutex::new(UltraLogLog::new(precision).expect("failed to create ull"));
